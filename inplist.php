...
<html>
<head>
	<title>Invalid Patient List</title>
...
<body>
	...
	Name</td><td align=center>Contact no.</td><td align=center>Validity</td></tr>
	<?php 
		$conn = mysqli_connect("localhost", "root", "", "hms");
		if (!$conn) {
			die("Connection failed: " . mysqli_connect_error());
		}
		$result = mysqli_query($conn, "SELECT * FROM patient p WHERE p.delete='n' ORDER BY p.pid");
		while($row = mysqli_fetch_assoc($result))
		{
			echo "<tr><td align=center>".$row['pid']."</td><td align=center>".$row['pfname']." ".$row['plname']."</td><td align=center>".$row['pmob']."</td><td align=center>Invalid</td></tr>";
		}
